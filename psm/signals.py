...
def update_purchase_store_contact(sender, instance, created=None, *args, **kwargs):
    if instance.status == 2:
        if instance.request_body == 1:
            ...
            district_store.save()
        if DistrictStore.objects.filter(district=instance.district, item=instance.item).exists():
            district_store = DistrictStore.objects.filter(district=instance.district, item=instance.item).first()
            district_store.stock_quantity += instance.quantity
            district_store.save()
        else:
            DistrictStore.objects.create(district=instance.district,item=instance.item,stock_quantity=instance.quantity)
